...
def slide(t):
    ...
    if t < start_time:
        return (vw, vh / 2 - h / 2)  # Start position: image is outside of screen on the right
    elif t < pause_start:
        ...
        x_slide_in = vw - (pause_start - start_time) / (pause_start - start_time) * w
        y = vh / 2 - h / 2  # Maintain the vertical center
        return (x_slide_in, y)
    else:
        progress = (t - pause_end) / (end_time - pause_end)
        # x = vw - progress * w  # Slide out towards the right side (optional)
        y = vh / 2 - h / 2  # Maintain the vertical center
        return (vw - w, y)
