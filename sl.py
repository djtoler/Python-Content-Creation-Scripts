...
def slide(t):
    ...
    if t < start_time:
        ...
        return (0, vh - h)  # Image stays at its final position
    else:
        progress = (t - pause_end) / (end_time - pause_end)
        x = -progress * (region_w)
        y = vh - (1 - progress) * (region_h)  # Start at the final position and slide downwards and to the left
        return (x, y)
...
def slide(t):
    ...
    # Width and height of the video
    vw, vh = video.size
    # Define the position where the image will slide in and pause
    slide_in_position = vw / 8
    max_slide_in_position = -vw / 4 + w  # Negate the value to start from outside the screen
    pause_position = slide_in_position
    # Time at which the slide should start, pause, and end
    start_time, pause_start, pause_end, end_time = 0, 4, 4 + pause_duration, total_duration
    if t < start_time:
        ...
        progress = (t - start_time) / (pause_start - start_time)
        x = -w + progress * max_slide_in_position  # Slide in from the left side
        y = vh / 2 - h / 2  # Start at the vertical center
        return (x, y)
    elif t >= pause_start and t < pause_end:
        return (slide_in_position, vh / 2 - h / 2)  # Image stays at the pause position
    else:
        progress = (t - pause_end) / (end_time - pause_end)
        x = max_slide_in_position - progress * max_slide_in_position  # Slide out towards the left side
        y = vh / 2 - h / 2  # Maintain the vertical center
        return (x, y)

# Make the image slide from left to right
image_slide = image.set_position(slide, relative=False)
